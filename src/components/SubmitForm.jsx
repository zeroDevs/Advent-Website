...
function SubmitForm({ name, handleClose }) {
  ...
  const handleSubmit = async event => {
    ...
    let body;
    try {
      body = JSON.stringify({ name, url, date }) // TODO: update property names to match expected body on backend
    } catch (error) {
  ...
  return (
    <>
      ...
      <Button variant="contained" color="secondary" onClick={handleSubmit} fullWidth>
        {isLoading ? <CircularProgress color="secondary" size={25} /> : "Submit"}
      </Button>
    </>
  )
};
